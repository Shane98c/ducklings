...
/** Allocated Wasm linear-memory capacity, not live allocations or total isolate memory. */
export function wasmMemoryBytes(): number { return module?.HEAPU8.byteLength ?? 0; }
...
export class PreparedStatement {
   ...
   */
  async run<T = Record<string, unknown>>(onDiagnostic?: (stage: string, wasmBytes: number) => void): Promise<T[]> {
    if (this.closed) throw new DuckDBError('Statement is closed');
    const mod = getModule();

    const checkpoint = (stage: string) => { try { onDiagnostic?.(stage, mod.HEAPU8.byteLength); } catch {} };
...
export class Connection {
  ...
  /**
   * Prepares SQL that may access remote metadata during binding.
   * Use this for HTTP/R2 Parquet sources: preparation itself can suspend.
   */
