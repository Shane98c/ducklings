...
for(const fault of ['none','execute','status','convert']){
 ...
 const stages=[];const promise=new Statement().run((stage,bytes)=>{assert.equal(bytes,128);stages.push(stage);});if(fault==='none')assert.deepEqual(await promise,[{col:'value'}]);else await assert.rejects(promise);
 if(fault==='none')assert.deepEqual(stages,['before-execute','after-execute','before-conversion','after-conversion','after-destroy']);
